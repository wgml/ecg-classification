...
struct NNAlgorithm {
  ...
  static void
  distance(const DataType &train_data, const DataType &test_sample, DistanceType &distances) {
    assert(test_sample.rows() == 1);
    assert(train_data.rows() == distances.rows());
    assert(train_data.cols() == test_sample.cols());
    distances = (DataType::Ones(train_data.rows(), 1) * test_sample -
                 train_data).array().square().rowwise().sum().sqrt().matrix();
//    distances = (train_data.rowwise() - test_sample).rowwise().squaredNorm().matrix();
  }
  ...
  static ClassType mode(const LabelType &labels) {
    ...
    for (int i = 0; i < labels.rows(); i++) {
      ...
      // matlab-compatible communist mode (favors lower class)
      if (o > top_value || (o == top_value && top_element > label)) {
        top_value = o;
        top_element = label;
      }
    }
